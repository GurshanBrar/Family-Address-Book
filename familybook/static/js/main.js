async function onload_function() {
    return new Promise(function (resolve) {
        let requestURL = 'http://localhost:8000/json/';
...
function initMap() {
    ...
    var map = new google.maps.Map(document.getElementById('entry-map'), {
        zoom: 1,
        center: center,
    });
...
async function processAddress(
    ...
    return new Promise(function (resolve) {
        geocoder.geocode({ address: addresses[i]['address'] }, function (
            results,
            status,
        ) {
            if (status === 'OK') {
                ...
                window.add_listener = function (div) {
                    ...
                    google.maps.event.addDomListener(div, 'click', function (
                        ...
                        resultsMap.setCenter(
                            resultslist[local_count].geometry.location,
                        );
...
function compute(map) {
    ...
    biggest_lat = Math.abs(
        Math.min(...total_lat_list) - Math.max(...total_lat_list),
    );
    biggest_lng = Math.abs(
        Math.min(...total_lng_list) - Math.max(...total_lng_list),
    );
